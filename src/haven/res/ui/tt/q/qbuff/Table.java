...
public class Table extends QList {
    ...
    public void layout(Layout layout) {
        ...
        for (Iterator it = this.ql.iterator(); it.hasNext(); ++i) {
            ...
            text[i] = CompImage.mk(Text.render(L10N.getString(L10N.Bundle.LABEL, qb.name) + ":").img);
            val[i] = CompImage.mk(Text.render((double) ((int) qb.q) == qb.q ? String.format("%d", (int) qb.q) : String.format("%.1f", qb.q)).img);
        }
